...
export default function UserDashboard() {
  ...
  const handleTerminalSubmit = async (e) => {
    ...
    const answerMap = {
      1: ["NTA="],
      2: ["TUlOVA==", "UElOSw==", "R09MRA==", "R1JBWQ==", "Q1lBTg=="],
      3: ["Mg=="],
      4: ["SEFDS0Q="],
      5: ["V0FMTEJSRUFDSEVE"],
  ...
  return (
    <div
      className={`min-h-screen font-mono overflow-x-hidden relative selection:bg-green-900/50 transition-colors duration-1000 ease-in-out ${isGameComplete ? "bg-slate-950 text-teal-400 selection:bg-teal-900/30" : "bg-black text-[#00ff00]"}`}
    >
      <header
        className={`fixed top-0 left-0 right-0 z-50 backdrop-blur-sm p-4 flex justify-between items-center shadow-lg transition-all duration-1000 ${isGameComplete ? "bg-slate-900/90 border-b border-teal-800/50 shadow-teal-900/20" : "bg-black/90 border-b border-green-900/50 shadow-[0_0_15px_rgba(0,255,0,0.1)]"}`}
      >
        <div className="flex items-center gap-2">
          <div className="flex gap-0.5 items-end h-5">
            {[1, 2, 3, 4].map((bar) => (
              <div
                key={bar}
                className={`w-1 transition-all duration-300 ${signalStrength >= bar ? "opacity-100" : "opacity-20"} ${isGameComplete ? "bg-teal-500" : "bg-[#00ff00]"}`}
                style={{ height: `${bar * 25}%` }}
              />
            ))}
          </div>
          <span
            className={`text-xs font-bold animate-pulse ${isGameComplete ? "text-teal-400" : "text-green-400"}`}
          >
        ...
        <div className="flex items-center gap-4">
          ...
          <Button
            ...
            className={`h-6 text-[10px] px-1 border border-transparent transition-colors ${isGameComplete ? "text-teal-700 hover:text-red-400 hover:bg-red-950/10 hover:border-red-900/30" : "text-green-800 hover:text-red-500 hover:bg-red-950/20 hover:border-red-900/50"}`}
          >
            <LogOut className="w-3 h-3 mr-1" /> DISCONNECT
          </Button>
          <div
            className={`text-xl font-bold tracking-widest text-right ${isGameComplete ? "text-teal-500/50" : "text-red-500 glitch"}`}
        ...
        <div className="text-right">
          <div
            className={`text-[10px] ${isGameComplete ? "text-teal-700" : "text-green-700"}`}
          >
      ...
      <main className="pt-24 pb-32 px-4 w-full max-w-md md:max-w-4xl mx-auto space-y-6 relative z-10 transition-all duration-300">
        <div className="flex flex-col md:flex-row gap-4 items-stretch h-auto md:h-[400px]">
          ...
          <div
            className={`hidden md:flex w-12 flex-col items-center justify-between py-2 border rounded-full relative overflow-hidden transition-colors duration-1000 ${isGameComplete ? "bg-slate-900 border-teal-900/30" : "bg-black border-green-900/30"}`}
          >
            <div
              className={`absolute inset-x-0 bottom-0 w-full ${isGameComplete ? "bg-teal-500/20" : "bg-green-500/20"}`}
              style={{ height: `${globalProgress}%` }}
            />
            <span
              className={`absolute bottom-4 w-full text-center text-xs font-bold z-20 ${isGameComplete ? "text-teal-400" : "text-green-400"}`}
            >
              {globalProgress}%
            </span>
          </div>
          ...
          <div
            className={`flex md:hidden w-full h-10 items-center border rounded-full relative overflow-hidden mb-6 mt-6 shrink-0 shadow-lg transition-colors duration-1000 ${isGameComplete ? "bg-slate-900 border-teal-900/50" : "bg-black border-green-900/50 shadow-[0_0_10px_rgba(0,255,0,0.1)]"}`}
          >
            <div
              className={`absolute inset-y-0 left-0 h-full transition-all duration-1000 ease-out z-0 ${isGameComplete ? "bg-teal-600/30" : "bg-green-600/30"}`}
              style={{ width: `${globalProgress}%` }}
            />
            <div
              className={`absolute inset-0 flex items-center justify-center text-xs font-bold z-10 drop-shadow-md ${isGameComplete ? "text-teal-400" : "text-green-400"}`}
            >
                ...
                return (
                  <Card
                    ...
                    className={`border-l-4 transition-all duration-300 backdrop-blur-md text-inherit ${
                      isGameComplete
                        ? "border-teal-500 bg-slate-900/40 shadow-teal-900/10 border-t-0 border-r-0 border-b-0 opacity-80"
                        : status === "active"
                    ...
                    <CardContent className="p-3 flex items-center justify-between">
                      <div>
                        <div className="flex items-center gap-2 mb-1">
                          {status === "active" ? (
                            <Zap
                              className={`w-3 h-3 animate-pulse ${isGameComplete ? "text-teal-400" : "text-yellow-400"}`}
                            />
                          ) : (
                            <CheckCircle
                              className={`w-3 h-3 ${isGameComplete ? "text-teal-400" : "text-green-400"}`}
                            />
                          )}
                          <span
                            className={`text-sm font-bold ${status === "completed" ? (isGameComplete ? "text-teal-300/80" : "line-through text-green-700") : isGameComplete ? "text-teal-100" : "text-green-400"}`}
                          >
                            {node.node_name ||
                        ...
                        <div
                          className={`text-xs uppercase ${isGameComplete ? "text-teal-600" : "text-green-600/80"}`}
                        >
                        ...
                        {status === "completed" &&
                          getSuccessMessage(node.id) && (
                            <div
                              className={`text-[10px] mt-1 font-mono break-all ${isGameComplete ? "text-teal-400/80" : "text-green-400/80"}`}
                            >
                              {getSuccessMessage(node.id)}
                            </div>
                          )}
                      ...
                      {status === "completed" && (
                        <span
                          className={`text-[10px] px-1 py-0.5 border rounded ${isGameComplete ? "bg-teal-900/10 text-teal-400 border-teal-500/20" : "bg-green-900/30 text-green-400 border-green-500/30"}`}
                        >
                          DECRYPTED
                        </span>
                      )}
        ...
        {team?.current_node === 4 && (
          <div
            className={`${isLogMaximized ? "fixed inset-0 z-50 bg-black p-8" : `relative border p-4 h-48 md:h-64 mb-6 ${isGameComplete ? "bg-slate-950 border-teal-900/30" : "bg-black border-green-900/50"}`} font-mono shadow-inner tracking-tight md:tracking-normal transition-all duration-300`}
          >
            <button
              onClick={() => setIsLogMaximized(!isLogMaximized)}
              className={`absolute top-24 right-4 p-3 rounded-full z-60 shadow-[0_0_15px_rgba(0,255,0,0.5)] transition-all hover:scale-110 border ${isGameComplete ? "bg-teal-900 border-teal-500 text-teal-400 hover:text-white" : "bg-green-900 border-green-500 text-green-500 hover:text-green-300"}`}
            >
              {isLogMaximized ? (
            ...
            <div
              className={`overflow-y-auto w-full ${isLogMaximized ? "h-full text-lg" : "h-full text-[10px] md:text-sm"} ${isGameComplete ? "text-teal-500/60" : "text-green-400"}`}
            >
        ...
        <div
          className={`border p-4 rounded-sm shadow-lg backdrop-blur-md transition-all duration-1000 ${
            isGameComplete
              ? "bg-slate-900/80 border-teal-500/30 shadow-teal-900/20"
              : `bg-black/80 ${errorMsg ? "border-red-500/50 shadow-red-900/20" : "border-[#00ff00]/30"}`
          }`}
        >
          <div
            className={`flex items-center gap-2 mb-3 border-b pb-2 transition-colors duration-1000 ${isGameComplete ? "text-teal-400 border-teal-900/50" : "text-[#00ff00] border-green-900/50"}`}
          >
          ...
          <div className="space-y-4">
            ...
            {team?.current_node === 3 && (
              <Button
                ...
                className={`w-full h-8 text-xs font-mono tracking-widest mb-2 animate-pulse border ${isGameComplete ? "bg-teal-900/20 border-teal-700 text-teal-400 hover:bg-teal-800/30" : "bg-green-900/20 border-green-700 text-green-400 hover:bg-green-800/30"}`}
              >
                [ SCAN_PHYSICAL_ASSET ]
              </Button>
            )}
            ...
            <div className="relative">
              <span
                className={`absolute left-3 top-2.5 text-sm select-none transition-colors duration-1000 ${errorMsg ? "text-red-500" : isGameComplete ? "text-teal-600" : "text-green-600"}`}
              >
                root@signal:~#
              </span>
              <Input
                ...
                className={`pl-32 font-mono h-10 transition-all duration-1000 ${
                  errorMsg
                    ...
                    : isGameComplete
                      ? "bg-slate-950 text-teal-400 border-teal-800 focus-visible:ring-teal-500 placeholder:text-teal-900"
            ...
            {isGameComplete && (
              <div className="text-center text-xs text-teal-500/60 font-mono animate-pulse">
      ...
      <Sheet open={isLeaderboardOpen} onOpenChange={setIsLeaderboardOpen}>
        <SheetTrigger asChild>
          <Button
            variant="outline"
            className={`fixed right-4 bottom-6 z-40 rounded-full h-12 w-12 p-0 shadow-lg transition-all hover:scale-105 ${isGameComplete ? "border-teal-500 bg-slate-900 text-teal-400 shadow-teal-900/40 hover:bg-teal-900/20" : "border-[#00ff00] bg-black text-[#00ff00] shadow-[0_0_20px_rgba(0,255,0,0.2)] hover:bg-green-900/20"}`}
          >
            <Trophy
              className={`w-5 h-5 ${isGameComplete ? "text-teal-300" : "text-yellow-400"}`}
            />
          </Button>
        </SheetTrigger>
        <SheetContent
          side="bottom"
          className={`h-[80vh] p-0 backdrop-blur-md ${isGameComplete ? "border-t-teal-500 bg-slate-900/95 text-teal-400" : "border-t-[#00ff00] bg-black/95 text-[#00ff00]"}`}
        >
          <SheetHeader
            className={`p-6 border-b ${isGameComplete ? "border-teal-900/50" : "border-green-900/50"}`}
          >
            <SheetTitle
              className={`font-mono flex items-center gap-2 ${isGameComplete ? "text-teal-400" : "text-[#00ff00]"}`}
            >
          ...
          <div className="p-6 space-y-4 overflow-y-auto max-h-[70vh]">
            {leaderboard.map((t, idx) => (
              <div
                key={t.id}
                className={`flex justify-between border-b pb-2 ${isGameComplete ? "border-teal-900/30" : "border-green-900/30"}`}
              >
